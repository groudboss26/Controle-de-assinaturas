...
import { SubscriptionModel, type Period, type Subscription, type SubscriptionInput } from '../models/Subscription.js';

const validPeriods: Period[] = ['weekly', 'monthly', 'yearly'];
...
function validatePayload(body: Record<string, unknown>): { input?: SubscriptionInput; error?: string } {
...
export class SubscriptionController {
  static create(req: Request, res: Response): void {
    ...
    const subscription = SubscriptionModel.create(input);
    res.status(201).json(subscription);
  }

  static getAll(_req: Request, res: Response): void {
    res.json(SubscriptionModel.getAll());
  }
  ...
  static update(req: Request, res: Response): void {
    ...
    const subscription = SubscriptionModel.update(id, input);
